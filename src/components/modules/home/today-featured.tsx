...
interface TodayFeaturedProps {
  className?: string;
}

export default async function TodayFeatured({ className }: TodayFeaturedProps) {
  const [recommendedData] = await Promise.all([fetchRecommendedProducts()]);

  const t = await getTranslations("FeaturedFinds");
  return (
    <div
      className={`w-full py-10 px-10 rounded overflow-hidden bg-[url('/home/today-features.webp')] bg-cover bg-center bg-no-repeat ${className}`}
    >
      <div className="">
        <SectionHeader
          title={
